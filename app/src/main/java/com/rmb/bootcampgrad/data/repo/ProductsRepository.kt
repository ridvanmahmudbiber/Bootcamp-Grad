...
class ProductsRepository(var productsDataSource: ProductsDataSource) {
    ...
    suspend fun loadProducts(category: String?): List<Products> =
        productsDataSource.loadProducts(category)

    suspend fun loadBasketProducts(): List<ProductsMyBasket>? =
        productsDataSource.loadBasketProducts()
    ...
    suspend fun addToBasket(
        ...
        productsDataSource.addToBasket(ad, resim, kategori, fiyat, marka, siparisAdeti)
}